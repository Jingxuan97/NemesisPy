...
def read_mre(runname,MakePlot=False):
    ...
    for i in range(nvar):
        ...
        varparam[i,:] = tmp[:]
        s = f.readline().split()
        varident1 = np.zeros([1,3],dtype='int32')
        varident1[:,:] = varident[i,:]
        varparam1 = np.zeros([1,5])
        varparam1[:,:] = varparam[i,:]
        np1 = npvar(1,npro,varident1,varparam1)
        nxvar[i] = np1
...
def read_ref(runname, MakePlot=False, SavePlot=False):
    ...
    if MakePlot == True:
        ...
        for j in range(ngas):
            strgas = spec.read_gasname(gasID[j],isoID[j])
            ax3.plot(vmr[:,j],height,label=strgas,linewidth=2.)

        ax3.set_xscale('log')
        ax3.set_xlabel('Volume mixing ratio')
        plt.subplots_adjust(left=0.06,bottom=0.12,right=0.88,top=0.96,wspace=0.16,hspace=0.20)
        legend = ax3.legend(bbox_to_anchor=(1.01, 1.02))
        ax1.grid()
        ax2.grid()
        ax3.grid()
        if SavePlot==True:
            fig.savefig(runname+'refatm.png',dpi=300)
...
def read_refc(runname, MakePlot=False, SavePlot=False):
...
#           ******** surface temperature
    ...
    return nvar,varident,varparam,jsurf,jalb,jxsc,jtan,jpre,jrad,jlogg,jfrac,nx,xa,sa,lx1
